...
<br><br>
<form method="post" id="form_file2" enctype="multipart/form-data">
    <div class="custom-file">
        <input type="file" name="file_kti" id="customFile2" class="custom-file-input" accept=".pdf">
        <input type="hidden" name="no_peserta" value="<?php echo $p->no_peserta; ?>">
        <label for="customFile2" class="custom-file-label">Pilih File Karya Tulis Ilmiah</label>
        <small id="file-error"></small>
    </div>
    <div class="form-group">
        <br>
        <button class="btn btn-primary" id="submit_file" type="button">Kirim File Karya Tulis Ilmiah</button>
    </div>
</form>
...
<script>
    $("#customFile2").change(function() {
        var file = this.files[0];
            ...
            if(!(file)){
                $(".custom-file-label").text("Pilih File Karya Tulis Ilmiah");
    ...
    $("#submit_file").click(function() {
        ...
        if(fileText.classList.contains("is-valid")){
            Swal.fire({
                title: "Konfirmasi Unggah File Karya Tulis Ilmiah",
                icon: "question",
                text: "File Karya Tulis Ilmiah Sudah Benar?",
                ...
                preConfirm: () => {
                    $.ajax({
                        type: "POST",
                        url: "akun/savekti",
                        data: new FormData(document.getElementById("form_file2")),
                        contentType: false,
                        cache: false,
                        processData: false,
                        beforeSend: function(){
                            $("#submit_file").attr("disabled","disabled")
                            $("#form_file2").css("opacity",".5")
                        },
                        success: function(response){
                            if(response == "OK"){
                                ...
                                Swal.fire({
                                    icon: "success",
                                    title: "Berhasil Mengunggah File Karya Tulis Ilmiah!",
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Gagal Mengunggah File Karya Tulis Ilmiah!',
